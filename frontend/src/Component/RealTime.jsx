...
export default function RealTime() {
  ...
  useEffect( () =>{
    ...
    const RealtimePlay = async(event) =>
    {
    ...
    const fetchVideo = async (event) => {
      ...
      const response = await axios.get(`http://localhost:5000/python/image/${id}`,{ responseType: 'blob' });
      console.log(response);
    ...
    //리스너생성
    input.addEventListener('click', RealtimePlay); //이게 있어야 drop 이 작동됨
  ...
  return (
    ...
    <SubTitle>실시간 동영상 촬영</SubTitle>
      ...
      {(datas.length===0) ? (<CenterText>실시간 촬영중입니다.</CenterText>) : null }
      {datas.map( (data,id) =>
        (
          <div key={id}>
            <div>&#123;</div>  
     ...
     { (datas.length===0) ? ( <CenterText>실시간 촬영중입니다.</CenterText>) : (
     <><p>얼굴 : {label[0]}</p>
     <p>휴대폰 : {label[1]}</p>
     <p>카드 : {label[2]}</p>
     <p>번호판 : {label[3]}</p></>) }
      ...
      <Button12 type="button" id ="btnSave" ref={saveRef}>완료된 실시간 영상을 보려면 클릭!</Button12>
